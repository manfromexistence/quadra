...
description: Create applications that integrate with Quadra using OAuth 2.0.
section: developer
order: 2
---

Build applications that let users connect their Quadra accounts. OAuth apps can access financial data on behalf of users, enabling integrations, automations, and custom tools.

## What you can build

With OAuth, you can build:

- **Browser extensions** that display Quadra data
- **Mobile apps** with Quadra integration
...
- **Public apps** listed in the Quadra app directory

## How OAuth works

OAuth 2.0 lets your app request access to a user's Quadra data without handling their password:

1. Your app redirects users to Quadra's authorization page
2. Users log in and approve the requested permissions
3. Quadra redirects back with an authorization code
4. Your app exchanges the code for access tokens
5. Use access tokens to call the Quadra API
...
```
https://app-quadra.vercel.app/oauth/authorize?
  response_type=code&
...
After the user authorizes, Quadra redirects to your `redirect_uri`:
...
```typescript
import { Quadra } from "@midday-ai/sdk";
...
app.get("/connect", (req, res) => {
  ...
  const authUrl = new URL("https://app-quadra.vercel.app/oauth/authorize");
