...
description: How Quadra automatically matches receipts and invoices to the right transactions, and what to expect in daily use.
section: inbox
order: 1
---

Quadra captures receipts and invoices, then matches them to your bank transactions so reconciliation takes minutes instead of hours.
...
2. Quadra extracts merchant, amount, date, and document details
...
Quadra uses multiple signals together:
...
- a suggestion when Quadra is likely right but wants confirmation,
...
Quadra is highly confident this is correct and attaches the receipt automatically.
...
Quadra thinks this is likely correct, but asks you to confirm.

Review and either:
- **Confirm**: Receipt is attached and matching gets smarter for similar cases
- **Reject**: Quadra records this as negative feedback
...
Quadra continuously improves through your team's actions:

### Merchant pattern learning

When your team repeatedly confirms the same merchant pairings, Quadra starts treating those as safer and faster to match.
...
When you reject a match, Quadra remembers that pair and avoids repeating it.

### Post-match learning

Even after confirmation, unmatching later is treated as negative feedback so the system avoids repeating similar mistakes.

## Getting receipts into Quadra

### Email forwarding

Every Quadra account has a unique inbox email address:
...
4. Quadra scans for receipts and invoices automatically
...
1. Connect your Slack workspace in [Inbox → Settings](https://app-quadra.vercel.app/inbox/settings)
2. Share any file to the connected channel
3. It appears in your Quadra inbox automatically
...
The purchase might be on an account you haven't connected. Check your connected accounts in [Settings → Accounts](https://app-quadra.vercel.app/settings/accounts).
...
2. Manually confirm a few correct matches so Quadra learns your pattern
...
- Connected email access is read-only—Quadra can't send emails
