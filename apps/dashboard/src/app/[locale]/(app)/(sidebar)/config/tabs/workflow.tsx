...
const WorkflowStep = memo(
  ({
    step,
    onEdit,
    onDelete,
  }: {
    step: ConfigWorkflowProps["workflowSteps"][0];
    onEdit: (step: ConfigWorkflowProps["workflowSteps"][0]) => void;
    onDelete: (id: string) => void;
  }) => (
    <div className="relative group">
      <div className="flex flex-col items-center gap-2 px-4 py-3 border border-border bg-card min-w-[140px]">
        <div className="font-semibold text-sm text-center">{step.stepName}</div>
...
export const ConfigWorkflow = memo(function ConfigWorkflow({
  projectId,
  workflowSteps,
  config,
}: ConfigWorkflowProps) {
  ...
  const openEditModal = useCallback((step: (typeof workflowSteps)[0]) => {
    setEditingStep(step);
    setIsModalOpen(true);
  }, []);
