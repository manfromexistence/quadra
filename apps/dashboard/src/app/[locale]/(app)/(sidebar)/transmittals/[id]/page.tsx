...
export default async function TransmittalDetailPage({
  ...
  const reviewApprovalOption = getClientApprovalOptionByCode(
      ...
      <section className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {data.transmittal.review ? (
              <div className="border border-border bg-card p-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <EdmsStatusBadge
                    status={data.transmittal.review.reviewStatus || "reviewed"}
                  />
                </div>
                <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                  ...
                  {data.transmittal.review.attachmentUrl ? (
                    <p>
                      CSR attachment:{" "}
                      <Link
                        href={expandStorageUrl(
                          data.transmittal.review.attachmentUrl,
                        )}
                        target="_blank"
                        className="text-primary hover:underline"
                      >
                        {data.transmittal.review.attachmentFileName ??
                          "Open attachment"}
                      </Link>
                    </p>
                  ) : null}
            ...
            {data.isRecipient &&
            data.activeWorkflowStep?.isActionable &&
            data.transmittal.status === "sent" ? (
            ...
            {data.isSender && data.transmittal.status === "reviewed" ? (
              <div className="border border-dashed border-border bg-muted/10 p-4 text-sm text-muted-foreground">
                Review has been returned. Acknowledge the package to close this
                cycle and continue with the next revision.
              </div>
            ) : null}

            {!data.isRecipient && !data.isSender ? (
              <p className="text-sm text-muted-foreground">
                You can view this package, but only the sender or recipient can
                act on it.
              </p>
            ) : null}
          </CardContent>
